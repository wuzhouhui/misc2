 ...
 */
/*
 * 物理内存布局. 为早期 PC 所做的设计决定, 导致内存被分成了 4 个主要区域:
 *  - 传统或基本内存区: 第一片 640KB, 传统区的开关部分参演用于 BIOS 存储
 *    数据
 *  - 较高的内存区(UMA): 内存第一兆的较高 384KB;
 *  - 高内存区(HMA): 内存第二兆的第一个 64KB;
 *  - 扩展内存区: 第一兆之外的所有内存.
 * 
 * 高内存区与扩展内存区的第一个 64KB 是重叠的, 但是与扩展内存的剩下的内存
 * 是不一样的, 因为在实模式下高内存区是可访问的.
 */
