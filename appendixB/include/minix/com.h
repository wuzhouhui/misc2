 ...
 */
/*
 * 某几个任务号的值取决于它们或其他任务是否被允许. 一般来说它们都是定义成
 * (PREVIOUS_TASK - ENABLE_TASK). ENABLE_TASK 不是0, 就是1,
 * 所以一个任务要么获得一个新任务号, 要么获得前一个任务的任务号, 而这个任务
 * 号在未来并不会被使用到(所以才能新任务号 --wzh). 注意, 在这里定义的任务
 * 号的顺序必须与 table.c 中定义的任务表相对应.
 */

/* Kernel tasks. These all run in the same address space. */
/* 内核任务, 它们都在同一个地址空间中运行. */
// 空闲任务, 当没有其他进程可运行时, 运行空闲任务, 类似于 Linux 中的
// 交换进程(任务 0).
