 ...
 */
/*
 * 这个子例程由 do_alarm() 调用, 用于设置闹钟定时器. 这个子例程同样也可以
 * 将定时器关闭, 如果一个进程退出的话.
 */
   ...
   */
  /*
   * 通知时钟任务, 在时间到来时, 提供一个信号消息.
   *
   * 大延迟会造成一系列的问题. 首先, 系统调用 alarm() 接受一个无符号数作为
   * 参数, 然后库函数会将其强制转化为有符号整数. 这有可能正常工作, 但是在
   * 返回时, 库函数会将 "负" 的无符号数转换为一个错误. 假设不对这些错误作
   * 检查, 于是在这里也不作检查. 第二, 如果无符号与长整数拥有相同的字长,
   * 从秒转化为滴答很容易溢出. 最后, 当滴答数增加时, 内核也会有类似的溢出
   * 漏洞.
   *
   * 为了修复这个漏洞, 需要大量的, 难看的强制类型转换. ALRM_EXP_TIME 拥有
   * 正确的类型(clock_t), 虽然 clock_t 造价于 long. 如何才能正确地声明类似
   * 这样的变量, 而不用暴露消息的类型? combinatorial(连带的)
   */
 ...
 */
/*
 * 向一个进程发送一个信号. 检查信号是否被捕捉, 忽略, 转换到一个消息中(对
 * 系统进程来说), 或 阻塞. 
 *   - 如果信息要被转换到一个消息中, 请求 KERNEL 向目标进程发送一个系统
 *     通知, 参数为未决的信号.
 *   - 如果信号会被捕捉, 请求内核向捕捉进程的栈中压入一个 sigcontext 
 *     结构与一个 sigframe 结构. 同时, KERNEL 复位程序计数器与栈指针, 
 *     于是进程下一次执行时就会执行信号处理函数. 当信号处理函数返回时,
 *     sigreturn(2) 就会被调用. 然后 KERNEL 就会从 sigcontext 结构中
 *     复原信号的上下文环境.
 * 如果栈空间不足, 杀死进程.
 */
