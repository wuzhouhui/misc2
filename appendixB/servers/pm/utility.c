 ...
 *
 * 入口点包括:
 *	find_param	查找一个引导监视器参数
 *	get_free_pid	获取一个空间的进程或进程组 ID 
 *	allowed		检查某个权限是否允许
 *	no_sys		无效系统调用
 *	panic		PM 遇到一个致命错误
 *	tell_fs		与文件系统的接口
 *	get_mem_map	获取指定进程的内存映射
 *	get_stack_ptr	获取指定进程的栈指针
 *	proc_from_pid	根据进程 ID 获取进程指针
 */
 ...
 */
/*
 * 检查文件是否具体由参数指定的权限. 如果不具有该权限, 返回 EACCES 或 
 * ENOENT. 如果权限正确, 打开文件并返回文件描述符.
 */
  int fd;
  int save_errno;
