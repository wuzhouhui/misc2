...
/* 读取文件头部数据, 提取文本段, 数据段, bss 段 和 文件总大小. */
   ...
   */
  /*
   * 读取文件头部数据并检查魔数. MINIX 的标准头部信息定义在 <a.out.h> 中,
   * 头部信息以 8 个字节开始, 后跟 6 个 long. 再接下去的 4 个 long 型字
   * 段在这里没有用到.
   *	字节 0:	魔数 0x01 
   *	字节 1:	魔数 0x03 
   *	字节 2: 一般情况下是 0x10(不检查该字段, 0 也允许), 分开的指令与
   *		数据则为 0x20 
   *	字节 3: CPU 类型, 16 位英特尔是 0x04, 32 位英特尔是 0x10, 摩托
   *		罗拉是 0x0b, sun SPARC 是 0x17 
   *	字节 4: 头部长度 0x20 
   *	字节 5-7: 不用
   *
   *	现在是 6 个 long 型字段.
   *	字节 8-11: 文本段的大小, 以字节计.
   *	字节 12-15: 已初始化的数据段的大小, 以字节计
   *	字节 16-19: bss 段大小, 以字节计
   *	字节 20-23: 程序入口点
   *	字节 24-27: 分配给程序的总的内存大小(text + data + stack)
   *	字节 28-31: 符号表大小, 以字节计
   *
   * long 型字段的字节充依赖于机器, 8088 小端, 68000 大端. 头部信息后面
   * 紧跟着文本段与数据段, 再是符号表(如果有的话). 段与表的大小在头部
   * 给出.只有文本段与数据段会被 exec() 复制到内存中. 头部数据在这里只
   * 使用一次. 符号表只用于调试, 在这里忽略符号表.
   */

  // 从文件中读取头部数据, 如果 shell 脚本文件, 则有可能只读取开头的两个
  // 字节(#!)
  ...
  // 在不是脚本文件的情况下, 如果读到的头部大小小于 A_MINHDR, 则出错返回
  if (m != A_MINHDR) return(ENOEXEC);

  /* Check magic number, cpu type, and flags. */
  if (BADMAG(hdr)) return(ENOEXEC);
  // 如果可执行文件的 cpu 字段不是 80386, 出错返回
  if (hdr.a_cpu != A_I80386) return(ENOEXEC);
  // 如果可执行文件的标志拥有除 (A_NSYM, A_SEP, A_EXEC) 之外, 其他的标志
  // 的话, 出错返回.
 ...
 */
/*
 * 寻找执行文件为 <ino, dev, ctime> 的进程. 不要 "意外" 的寻找 mp_ign, 
 * 因为它是产生这个调用的进程.
 */
  struct mproc *sh_mp;
