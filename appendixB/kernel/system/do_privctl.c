 ...
 */
/*
 * 处理 sys_privctl(). 更新一个进程的特权级结构. 如果进程不是一个系统进程,
 * 确保它拥有自己的一个特权级结构.(用户进程共享同一个特权级结构)
 */
   ...
   */
  /*
   * 检查主调进程是否有权限调用该系统调用. 带特权的进程只能更新那些由于设置
   * 了 NO_PRIV 而无法运行的进程的特权级结构. 当一个带特权的进程派生出子进
   * 程时, 会设置 NO_PRIV 标志.
   */
   ...
   */
  /*
   * 确保这个进程拥有自己的特权级结构. 这有可能失败, 因为只有有限数量的系统
   * 进程. 然后从主调进程复制出特权级结构, 再复原一些默认值.
   */
