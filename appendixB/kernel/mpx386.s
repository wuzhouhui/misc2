...
! 落入该代码, 来重新启动 进程/任务 运行

!*===========================================================================*
!*				restart					     *
!*===========================================================================*
! 载入某个进程所有保存的寄存器, 并使之运行
_restart:

! Restart the current process or the next process if it is set. 
! 重新启动当前进程, 或下一个进程(如果有的话)

	! 查看下一个可运行进程是否已设置, 如果没有跳到标号 0
	! 否则将下一个可运行进程作为当前运行进程.
	...
	mov	(_next_ptr), 0
	! 下面这条指针是把 _proc_ptr 放到寄存器中, 还是放到内存中???
	! 使用之前不用先保护一下吗???
	! 应该是切换栈, 进程结构体定义刚开始就是栈帧
	! this is a nasty trick to load the registers really fast. the 
	! stack pointer is set to the process table entry of the process 
	! to run(proc_ptr) and the registers are simply loaded with 'pop'
	! instruction.
0:	mov	esp, (_proc_ptr)	! will assume P_STACKBASE == 0
	! P_LDT_SEL ??? 应该是根据进程指针, 获取进程的局部段描述符, 进而
	! 局部段描述符的选择符
...
restart1:
	decb	(_k_reenter)
	! o16: indicates that the instruction should be assembled into 
	! an instruction using 16-bit data.
    o16	pop	gs
    o16	pop	fs
    o16	pop	es
    o16	pop	ds
        ! pop all general registers. including edi, esi, esi, ebp, ebx,
	! edx, ecx and eax.
	popad
