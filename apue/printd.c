 ...
 *       Filename:  printd.c
 *
 *    Description:  print spooler daemon
 *
 *        Version:  1.0
 *        Created:  01/19/2015 03:08:03 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
...
/*
 * describe a thread processing a client request.
 */
...
int main(int argc, char *argv[])
{
	...
	if ((err = getaddrlist(host, "print", &ailist)) != 0) {
		log_quit("getaddrlist error: %s", gai_strerror(err));
		exit(1);
	}
	...
	if (pwdp->pw_uid == 0)
		log_quit("user %s is priviledged", LPNAME);
	...
	for (;;) {
		...
		for (i = 0; i <= maxfd; i++) {
			if (FD_ISSET(i, &rset)) {
				/*
				 * accept the connection and handle the
				 * request
				 */
 ...
 *
 * LOCKING: acquires and release joblock.
 */
...
void *signal_thread(void *arg)
{
	...
	for (;;) {
		...
		default:
			kill_workers();
			log_quit("unexpected signal %s", signo);
		}
	}
}
...
void *printer_thread(void *arg)
{
	...
	for (;;) {
		...
		if ((sockfd = connect_retry(AF_INET, SOCK_STREAM, 0,
			...
			log_msg("job %d defered - can't contact printer: %s",
		...
		*hcp++ = '\n';
		...
		if (jp->req.flags & PR_TEXT) {
			/*
			 * hack: allow PostScript to be printed as plain
			 * text.
			 */
		...
		if (printer_status(sockfd, jp)) {
			unlink(name);
			printf(name, "%s/%s/%d", SPOOLDIR, REQDIR, jp->jobid);
			unlink(name);
			free(jp);
			jp = NULL;
		}
